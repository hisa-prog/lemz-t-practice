...
import { batteryPack } from "../../data/battery";
...
const Battery = () => {
  ...
  useEffect(() => {
    if(batteryPack.alarm 
      || batteryPack.durability <= 35
      || batteryPack.charge_level <= 50) setCriticalState(true)
    ...
    return (
      <div className="flex flex-col w-full h-fit">
        ...
        color={batteryPack.durability >= 75 ? 'text-green-400' 
        : batteryPack.durability >= 35 ? 'text-yellow-500' 
        : 'text-red-500'}
        procent
        />
        <BatteryStatString
        ...
        color={batteryPack.charge_level >= 75 ? 'text-green-400' 
        : batteryPack.charge_level >= 35 ? 'text-yellow-500' 
        : 'text-red-500'}
        procent
        />
        <BatteryStatString
        ...
        <div className="grid grid-cols-8 gap-1.5 mt-4">
          {batteryPack.capacitors.map((item, index) => (
            <BatteryCapasitorCol
              ...
              firstColor={item.durability === 100 ? 'text-green-400' 
              : 'text-red-500'}
              secondVal={item.charge_level}
              secondColor={item.charge_level >= 99.5 ? 'text-green-400' 
              : item.charge_level >= 35 ? 'text-yellow-500' 
              : 'text-red-500'}
            />
          ))}
        </div>
